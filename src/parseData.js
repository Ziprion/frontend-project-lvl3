export default (state, data, urlRSS) => {
  ...
  state.feeds.push({ title: titleFeedText, url: linkFeed, description: descriptionFeedText, id: feedID });
  ...
  const postsItem = Array.from(items).reduce((acc, item) => {
    ...
    return [...acc, { id: itemID, title: normalTitleItemText, href: hrefText, description: descriptionText }];
  }, []);
